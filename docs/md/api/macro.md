...
```bash
curl -X GET 'http://localhost:3010/macros?clientId=fvtt_d3db244ce1d8af0b' \
...
{
  "type": "macros-result",
  "requestId": "macros_1780520794707",
  "macros": [
    {
...
```javascript
const baseUrl = 'http://localhost:3010';
const path = '/macro/Macro.3bMxRd6G3ULP6qKD/execute';
const params = {
  clientId: 'fvtt_d3db244ce1d8af0b'
};
const queryString = new URLSearchParams(params).toString();
...
```bash
curl -X POST 'http://localhost:3010/macro/Macro.3bMxRd6G3ULP6qKD/execute?clientId=fvtt_d3db244ce1d8af0b' \
  -H "x-api-key: your-api-key-here" \
...
base_url = 'http://localhost:3010'
path = '/macro/Macro.3bMxRd6G3ULP6qKD/execute'
params = {
    'clientId': 'fvtt_d3db244ce1d8af0b'
}
url = f'{base_url}{path}'
...
🏁 🍇
  ...
  🔤localhost🔤 ➡️ host
  3010 ➡️ port
  🔤/macro/Macro.3bMxRd6G3ULP6qKD/execute🔤 ➡️ path
  ...
  💭 Build HTTP request
  🔤POST /macro/Macro.3bMxRd6G3ULP6qKD/execute🧲queryString🧲 HTTP/1.1❌r❌nHost: localhost:3010❌r❌nx-api-key: your-api-key-here❌r❌nContent-Type: application/json❌r❌nContent-Length: 66❌r❌n❌r❌n🧲body🧲🔤 ➡️ request
...
```json
{
  ...
  "requestId": "macro-execute_1780520794712",
  "uuid": "Macro.3bMxRd6G3ULP6qKD",
  "success": true,
  "result": {
    "success": true,
